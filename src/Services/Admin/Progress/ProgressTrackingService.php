...
readonly class ProgressTrackingService implements ProgressTrackingServiceInterface
{
    ...
    private function getUsersWithPagination(?string $searchTerm, int $page, int $limit, array $accessCriteria): array
    {
        ...
        if ($this->progressAccessService->isSuperAdmin()) {
            ...
            if ($currentUserService) {
                ...
                $stmt->bindParam(':serviceId', $binaryUuid, \PDO::PARAM_STR);
                ...
                if (empty($allUsers)) {
                    ...
                    $serviceStmt->bindParam(':serviceId', $binaryUuid, \PDO::PARAM_STR);
     ...
     * Cette méthode permet de rechercher des utilisateurs par prénom, nom, email ou NNI.
     *
        ...
        return array_filter(
            ...
            callback: static function (User $user) use ($normalizedSearchTerm): bool {
                ...
                return
                    ...
                    || ((null !== $nniNormalized) && str_contains(haystack: $nniNormalized, needle: $normalizedSearchTerm));
            }
        );
    }
    ...
    public function getThemeProgressData(): array
    {
        ...
        return $this->themeProgressService->getThemeProgressData($typedAccessCriteria);
    }
}
